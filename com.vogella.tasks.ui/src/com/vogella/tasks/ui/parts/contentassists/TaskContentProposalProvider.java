...
public class TaskContentProposalProvider implements IContentProposalProvider {
	...
	public IContentProposal[] getProposals(String contents, int position) {
		...
		IContentProposal[] filteredProposals = tasks.stream().filter(
				task -> task.getSummary().length() >= substring.length() && pattern.matcher(task.getSummary()).find())
				.map(task -> new TaskContentProposal(task.getSummary() + " --- " + task.getDescription(), task))
				.toArray(TaskContentProposal[]::new);
